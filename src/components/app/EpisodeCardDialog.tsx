...
const OverlayItem = ({ icon: Icon, count, onClick, isAction = false, isLiked = false, loading = false }: any) => {
  ...
  return (
    <div className={`${baseClass} ${actionClass}`} onClick={onClick}>
      {loading ? (
        <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
      ) : (
        <Icon size={count !== undefined ? 16 : 24} className={iconColorClass} />
      )}
      {count !== undefined && !loading && <span className="text-xs">{count}</span>}
    </div>
  );
};
...
export function EpisodeDialog({ episodeId, isOpen  }: { episodeId: string; isOpen: boolean; }) {
  ...
  return (
    <DialogContent 
      ...
      style={{ width: `${PORTRAIT_WIDTH}px`, height: `${PORTRAIT_HEIGHT}px` }}
    >
      <div className="relative w-full h-full">
        <div className={`absolute inset-0 transition-opacity duration-300 ${showComments ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          ...
          <div className="absolute top-0 left-0 p-4 pointer-events-none">
            <div className="flex flex-col gap-2 pointer-events-auto">
              ...
              <OverlayItem icon={FaDownload} count={episode.downloads} onClick={handleDownload} isAction loading={isDownloading} />
              <OverlayItem icon={FaCommentDots} onClick={() => setShowComments(true)} isAction />
            </div>
          </div>
          {user?._id === episode.userId._id && <Button onClick={handleToggleVisibility} size="sm" className="absolute bottom-10 right-4">
            {episode.visibility === "onlyme" ? "Set Public" : "Set Private"}
          </Button>}
        </div>

        <div className={`absolute inset-0 bg-white dark:bg-neutral-900 rounded-lg flex flex-col transition-transform duration-300 ${showComments ? 'translate-x-0' : 'translate-x-full'}`}>
